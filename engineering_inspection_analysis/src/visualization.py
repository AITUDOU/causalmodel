...
# 设置中文字体支持
# 与 concrete_causal_demo 保持一致，优先 Arial Unicode MS，其次 SimHei/DejaVu
...
def plot_causal_graph(graph: nx.DiGraph,
    ...
    if label_map is not None:
        labels = {node: label_map.get(node, str(node)) for node in graph.nodes}
...
def plot_attribution_results(attribution_df: pd.DataFrame = None,
    ...
    if attribution_df is not None:
        ...
        # 规范化置信区间，避免上下界颠倒
        lower_ci = np.minimum(df['Lower_CI'].astype(float), df['Upper_CI'].astype(float))
        upper_ci = np.maximum(df['Lower_CI'].astype(float), df['Upper_CI'].astype(float))

        # 计算非负误差条
        contrib = df['Contribution'].astype(float)
        lower_err = (contrib - lower_ci).clip(lower=0).fillna(0)
        upper_err = (upper_ci - contrib).clip(lower=0).fillna(0)
...
def plot_intervention_results(result_df: pd.DataFrame,
    ...
    # 计算非负误差条
    lower_err = (df['Causal_Effect'].astype(float) - lower_ci).clip(lower=0).fillna(0)
    upper_err = (upper_ci - df['Causal_Effect'].astype(float)).clip(lower=0).fillna(0)
